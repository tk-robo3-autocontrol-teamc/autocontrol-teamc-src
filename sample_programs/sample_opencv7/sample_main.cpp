...
/ 関数の宣言
std::pair<int, int> processFrames(RsCamera& camera, cv::Mat& img, cv::Mat& dph_img);
...
int main() {
    ...
    try {
        auto [x_width, depth] = processFrames(camera, img, dph_img);
        std::cout << "Width: " << x_width << ", Depth: " << depth << std::endl;
...
std::pair<int, int> processFrames(RsCamera& camera, cv::Mat& img, cv::Mat& dph_img) {
    ...
    while (true) {
        ...
        if (min_point[2] < 1.0) {
            x_width = max_point[0] - min_point[0];
            depth = max_point[2];
            break;
        }
