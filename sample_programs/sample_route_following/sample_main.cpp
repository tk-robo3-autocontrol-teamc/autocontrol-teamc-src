...
int main(int argc, char *argv[])
{
    ...
    while(1) {
        ...
        last_pls_L = drive.GetPositionIs(drive.iNodeIDLeft);// 車輪位置の取得1
        last_pls_R = drive.GetPositionIs(drive.iNodeIDLeft);

        VCS_MoveWithVelocity(drive.hKeyHandle, drive.iNodeIDLeft,  -ul, &drive.ErrorCode);
        VCS_MoveWithVelocity(drive.hKeyHandle, drive.iNodeIDRight,  ul, &drive.ErrorCode);
